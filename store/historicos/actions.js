...
const actions = {
  nuxtServerInit (vuexContext, context) {
    return context.$axios
      .$get(URI_BASE_API + API_VERSION + '/projetos')
      .then((data) => {
        vuexContext.commit('projetos/inserir_projetos', data)
      })
  },

  async criarHistorico (vuexContext, params) {
    return await new Promise((resolve, reject) => {
      this.$axios.$post(URI_BASE_API + API_VERSION + '/historicos', params)
        .then((response) => {
          vuexContext.commit('adicionar_historico', response.data)
